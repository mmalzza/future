...
#### 에러 메세지 - 이미 신청된 강의
![이미](./images/4.이미신청된강의.png)

#### 에러 메세지 - 정원 초과
![초과](./images/4.정원초과.png)
...
- 중복 신청 검증은 이메일 + courseId 조합 기준으로 처리
- 정원 초과 검증은 Mock 강의 데이터를 기준으로 처리
- 신청 완료 데이터는 서버 재시작 시 초기화
- 브라우저 새로고침 시 작성 중인 데이터 유지 기능은 이탈 경고 방식으로 UX를 보완
- 나의 강의실, 관심 프로그램 저장, 실제 신청 내역 조회 기능은 향후 확장 기능으로, 현재 범위에서는 구현하지 않았습니다.
...
에러 코드를 구현하여 실제 서버 환경과 유사한 예외 처리 흐름을 테스트할 수 있도록 구성하였습니다.
